...
2. **配置加载顺序**
   ...
   - 本地application.yml → 立即生效，设置启动参数
   - 远程application.yml → 后期加载（此时debug已错过生效时机）
...
1. **加了 @RefreshScope 的 Bean** → 不需要重启，会自动刷新
2. **普通 Bean** → 需要重启，配置变更影响不了已创建的 Bean
3. **连接配置中心的地址变更** → 必须重启
4. **bootstrap.yml 中的配置** → 必须重启

总结：只有 `@RefreshScope` 注解的 Bean 能热更新，其他都得重启。
   ```
...
→ [回答历史](../../../series/答题历史/框架/springcloud-答题记录.md#服务-a-feign-调用服务-bb-挂了怎么发现和兜底)
