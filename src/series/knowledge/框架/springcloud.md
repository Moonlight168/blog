...
- **熔断**：下游服务调用失败率达到阈值 → 熔断器打开 → 后续请求**快速失败**不调下游 → 一段窗口期后半开试探 → 成功则关闭。保护自己不被下游拖垮。
- **降级**：服务不可用时**返回兜底结果**（fallback），如返回缓存数据、默认值、空列表，保证主流程不受影响。
- **限流**：控制 QPS/并发数，超过阈值直接拒绝，保护系统不被冲垮。

### 熔断三种状态

关闭（正常）→ 开启（熔断中，快速失败）→ 半开（试探一次，成功则关闭，失败则继续开启）
