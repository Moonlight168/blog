...
| | Minor GC | Full GC |
|---|---|---|
| 发生区域 | 年轻代（Eden 满了） | 整个堆（年轻代+老年代+元空间） |
| 频率 | 频繁 | 应尽量避免 |
| STW 时间 | 短 | 长 |
| 算法 | 复制算法 | 标记-清除/标记-整理 |
...
1. **老年代空间不足** — 对象晋升老年代时剩余空间放不下
2. **元空间不足** — 类加载过多 / 反射生成过多类
3. **显式调用** `System.gc()` — 只是建议，不保证执行
4. **大对象直接进老年代空间不够** — 大于 `-XX:PretenureSizeThreshold` 的对象
5. **空间分配担保失败** — Minor GC 前检查老年代连续可用空间是否大于新生代所有对象总大小，不够则触发保障
...
- **线程私有：**
  - **程序计数器**：当前线程执行字节码行号，唯一不 OOM 的区域
  - **虚拟机栈**：每个方法一个栈帧（局部变量表、操作数栈、返回地址），超深 StackOverflowError
  - **本地方法栈**：Native 方法调用
- **线程共享：**
