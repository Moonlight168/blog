...
OM指Java中的`OutOfMemoryError`（内存溢出错误），是JVM无法处理的严重错误，会导致程序崩溃。

- **类型**：属于`Error`体系
- **本质**：JVM内存区域（堆、栈、方法区等）耗尽
- **表现**：程序突然崩溃，打印错误堆栈
...
| 类型 | 原因 | 错误信息 | 常见场景 |
|------|------|----------|----------|
| **堆内存溢出**（最常见） | 对象过多且无法回收，堆内存耗尽 | `OutOfMemoryError: Java heap space` | 内存泄漏、大数据处理、无限循环创建对象 |
| **栈内存溢出** | 方法调用栈过深，栈帧无法创建 | `StackOverflowError` | 无限递归、调用链过长 |
| **方法区/元空间溢出** | 类加载过多或动态生成类过多 | `PermGen space`（JDK8前）或`Metaspace`（JDK8+） | 大量动态生成类、框架过度使用 |
| **直接内存溢出** | NIO直接内存分配过多 | `OutOfMemoryError: Direct buffer memory` | 大量使用`ByteBuffer.allocateDirect()`、网络大缓冲区 |
| **其他原因** | 线程过多、JNI内存泄漏、JVM参数不合理 | 多种错误信息 | 大量线程创建、本地方法内存泄漏、堆内存设置过小 |
