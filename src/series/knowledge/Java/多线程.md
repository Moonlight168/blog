...
用户态和内核态是CPU的两种工作状态，用于隔离不同特权级别的代码执行：

### 1. 用户态（User Mode）

- **特权级别**：低特权级别
- **执行代码**：用户程序代码
- **资源访问**：只能访问用户空间的资源，不能直接访问内核空间
- **系统调用**：需要通过系统调用（如`syscall`）进入内核态才能访问内核资源
- **安全性**：即使程序崩溃，也不会影响操作系统

### 2. 内核态（Kernel Mode）

- **特权级别**：高特权级别
- **执行代码**：操作系统内核代码
- **资源访问**：可以访问所有系统资源（内存、I/O设备、CPU寄存器等）
- **系统调用**：直接执行内核指令
- **安全性**：程序崩溃可能导致整个系统崩溃
...
这种设计可以保护操作系统的安全性和稳定性，防止用户程序直接访问敏感资源。
