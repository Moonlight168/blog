...
系统结合 **Camunda 8 (Zeebe)** 工作流引擎、**Spring Cloud Alibaba 微服务架构** 与 **LLM 智能决策 Agent**，用于跨部门预算、资源申请等业务流程的自动化与智能化。
...
| 工作流引擎 | **Camunda 8 (Zeebe)**                 | 分布式 BPMN 工作流引擎 |
...
| 模块                   | 技术栈                        | 职责          |
| -------------------- | -------------------------- | ----------- |
| approval-service     | Spring Boot + MyBatis-Plus | 审批处理逻辑、流程调用 |
| resource-service     | Spring Boot + JPA          | 资源调配与记录     |
| notification-service | Spring Boot + Mail/WS      | 通知推送、消息广播   |
| auth-service         | Spring Security + JWT      | 鉴权与用户管理     |
| gateway-service      | Spring Cloud Gateway       | 接口网关与流量控制   |
| common模块             | DTO、Feign Client、Exception | 通用依赖模块      |
...
| 模块       | 技术             | 说明          |
| -------- | -------------- | ----------- |
| 主存储      | PostgreSQL 15+ | 结构化业务数据     |
| 缓存       | Redis 7+       | 会话与高频查询缓存   |
...
| 模块   | 技术            | 说明          |
| ---- | ------------- | ----------- |
| 指标采集 | Prometheus    | 性能指标与健康状态   |
| 链路追踪 | SkyWalking    | 服务调用与耗时监控   |
| 可视化  | Grafana       | 图表与告警面板     |
| 日志聚合 | ELK / Loki    | 集中式日志分析     |
| 运行管理 | Zeebe Operate | BPMN 流程实例监控 |
...
| 工作流 | Zeebe Cluster     | Partition 迁移与无状态恢复 |
...
| 模块       | 优化方向                              |
| -------- | --------------------------------- |
| 智能 Agent | 支持多模型协作 (OpenAI + Claude + 本地LLM) |
| 工作流      | 集成 Camunda Form 表单与决策表            |
| 微服务治理    | 引入 Service Mesh（Istio）            |
| 安全体系     | 支持 OAuth2 + 企业单点登录                |
| DevOps   | 接入 ArgoCD，实现 GitOps 持续交付          |
