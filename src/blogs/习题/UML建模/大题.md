...
#### UML 1.0 包含的图
UML1.x版本中包含9种图形，分别是：
- **用例图（Use Case Diagram）**：描述系统功能和参与者关系
- **类图（Class Diagram）**：描述类之间的关系
- **对象图（Object Diagram）**：类图的实例化
- **状态图（State Diagram）**：描述对象的状态变化
- **活动图（Activity Diagram）**：描述业务流程
- **顺序图（Sequence Diagram）**：描述对象间的消息交互时序
- **协作图（Collaboration Diagram）**：描述对象间的协作关系
...
3. **其他修改**：将序列图和协作图统一为交互图，增强了活动图的功能

#### 静态图
- 类图、对象图、组件图、部署图、包图、组合结构图

#### 动态图
- 用例图、序列图（通信图）、状态图、活动图、交互概览图、时序图
