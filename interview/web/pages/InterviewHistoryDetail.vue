...
import { durationLabel, resumeLabel, statLabel, timeLabel, topicLabel } from "../lib/format";
...
async function removeSession() {
  ...
  deleting.value = true;
  try {
    await api(API.interviewDiscard(String(route.params.id)), { method: "POST" });
...
async function refile(attempt: Attempt) {
  ...
  try {
    const data = await api<{ historyUrl: string; sourcePath: string }>(
      API.interviewArchiveAnswer(String(route.params.id), attempt.id), { method: "POST" });
...
onMounted(async () => { try { detail.value = await api(API.interviewReport(String(route.params.id))); } finally { loading.value = false; } });
</script>
