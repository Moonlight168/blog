...
import type { Chapter, Message, Session, TopicSeries } from "../../shared/types.ts";
import { API } from "../../shared/routes.ts";
import { useSpeechSynthesis } from "../composables/useSpeechSynthesis";
import { EVENT } from "../../shared/events.ts";

const toast = useMessage();
// 模型输出一律按 markdown 渲染（含中文紧贴加粗的处理，见 web/markdown.ts）
...
// 只列 HTML 简历本体，显示名去掉后缀（如「张三-Java后端(AI应用)-27届」）
...
const secondsLeft = computed(() => {
  ...
  // 与 server/modules/interview/timer.ts 同一套规则：扣除累计暂停与「当前这次」暂停。
...
async function loadBootstrap() {
  loading.value = true;
  try {
    const data = await api<{ resumeDir: string; resumes: typeof resumes.value; jobs: typeof jobs.value; topics: TopicSeries[]; embeddingEnabled: boolean; asrEnabled?: boolean }>(API.settings);
...
async function transcribeBlob(blob: Blob) {
  ...
  try {
    ...
    const data = await api<{ text: string }>(API.speechTranscribe, { method: "POST", headers: { "Content-Type": "audio/wav" }, body: wav });
 ...
 */
const {
  ttsSupported, autoSpeak, speakingId, speakingPaused, spokenThrough, speakRate, SPEAK_RATE_OPTIONS,
  isSpeaking, speak, stopSpeak, toggleSpeak,
} = useSpeechSynthesis(messages);
// 新消息到达时自动朗读、关掉开关时停止当前条：都在 useSpeechSynthesis 里（它自带 watch）。
...
async function openPreview() {
  ...
  try {
    const data = await api<{ markdown: string; chars: number }>(API.resumesPreview, {
      method: "POST",
...
async function openIntro(file = "") {
  ...
  try {
    ...
    const data = await api<{ files: { file: string; name: string }[]; file: string; markdown: string; exists: boolean }>(
      `${API.document("self-intro")}${wanted ? `?file=${encodeURIComponent(wanted)}` : ""}`,
    );
...
async function saveResumeDir() {
  ...
  try {
    const data = await api<{ resumeDir: string; resumes: typeof resumes.value; jobs: typeof jobs.value }>(API.resumeDir, {
      method: "POST",
...
async function restore() {
  ...
  if (!id) return;
  try {
    const data = await api<{ session: Session; messages: Message[] }>(API.interview(id));
...
async function syncExpiredSession() {
  ...
  try {
    const data = await api<{ session: Session; messages: Message[] }>(API.interview(session.value.id));
    session.value = data.session; messages.value = data.messages;
...
async function start() {
  ...
  sending.value = true;
  try {
    const data = await api<{ session: Session; messages: Message[] }>(API.interviews, { method: "POST", body: JSON.stringify({ resumePath: resumePath.value, series: series.value, chapterPath: chapterPath.value, mode: mode.value, durationMinutes: durationMinutes.value, jdPath: jdPath.value }) });
    session.value = data.session; messages.value = data.messages;
...
async function submit(body: Record<string, unknown>, restoreDraft?: () => void) {
  ...
  try {
    ...
    const data = await apiStream<{ session: Session; messages: Message[] }>(API.interviewMessages(session.value.id), {
      method: "POST", body: JSON.stringify({ ...body, stream: true }), signal: activeRequest.value?.signal,
    }, (event) => {
      if (event.type === EVENT.stage) {
        aiStage.value = event.stage === "archiving" ? "正在保存本次回答…" : "AI 正在处理…";
      }
      if (event.type === EVENT.tool) {
        // 工具名来自 pi：read / bash 是查资料，edit / write 是整理想法
        aiStage.value = event.name === "read" || event.name === "bash" ? "AI 正在查阅资料…" : "AI 正在整理结果…";
      }
      if (event.type === EVENT.draft && event.content) {
        // 收到的是**增量片段**，必须拼接。早期服务端推的是累积整段、前端用赋值接，
        // 逐字之后若还赋值，气泡里就只剩最后一个字。
      ...
      if (event.type === EVENT.evaluation && event.evaluation) {
        messages.value = [...messages.value.filter((item) => !item.pending), {
...
async function togglePause() {
  ...
  const wasPausing = paused.value;
  pausing.value = true;
  try {
    const endpoint = wasPausing ? API.interviewResume(session.value.id) : API.interviewPause(session.value.id);
    const data = await api<{ session: Session; messages: Message[] }>(endpoint, { method: "POST" });
    session.value = data.session;
    toast.success(wasPausing ? "继续面试" : "已暂停，计时停住了");
  } catch (error) { toast.error((error as Error).message); }
...
async function endAndDiscard() {
  ...
  try {
    await api(API.interviewDiscard(session.value.id), { method: "POST" });
