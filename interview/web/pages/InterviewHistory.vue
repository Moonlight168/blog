...
import { api } from "../api";
import { SCORE_BANDS, durationLabel, personLabel, resumeLabel, statLabel, timeLabel, topicLabel } from "../lib/format";
...
onMounted(async () => {
  try { rows.value = await api<Row[]>(API.interviews); } finally { loading.value = false; }
...
async function removeRow(row: Row) {
  ...
  try {
    await api(API.interviewDiscard(row.id), { method: "POST" });
