...
onMounted(async () => {
  try {
    const data = await api<{ embeddingEnabled: boolean; embeddingModel: string | null }>(API.settings);
...
async function runSearch() {
  ...
  loading.value = true;
  try {
    const data = await api<SearchResult>(`${API.questionsSearch}?query=${encodeURIComponent(text)}&topK=5`);
    result.value = data;
    embeddingEnabled.value = data.embeddingEnabled;
    embeddingModel.value = data.embeddingModel;
  } catch (error) {
...
async function manualRefresh(force: boolean) {
  ...
  try {
    const data = await api<RefreshResult>(API.questionsReindex, {
      method: "POST",
