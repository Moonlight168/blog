...
import type { Chapter, TopicSeries } from "../../shared/types.ts";
...
onMounted(async () => {
  try {
    const data = await api<{ topics: TopicSeries[]; docsBaseUrl: string }>(API.settings);
...
async function draw() {
  ...
  loading.value = true;
  try {
    const data = await api<{ questions: ReviewQuestion[] }>(API.questionsPick, {
      method: "POST",
...
async function toggleHistory(question: ReviewQuestion) {
  ...
  try {
    const data = await api<{ entries: HistoryEntry[] }>(
      `${API.questionsAnswers}?chapterPath=${encodeURIComponent(chapterPath.value)}&title=${encodeURIComponent(question.title)}`);
