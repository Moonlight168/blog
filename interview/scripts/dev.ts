...
async function waitForApi(port: number): Promise<boolean> {
 ...
 * 先起后台，等它真的开始监听了再起前端。
 *
 * 为什么不能同时起：server/index.ts 在 listen **之前**要先跑一遍题库索引刷新（会打向量服务），
 ...
 */
const api = spawn(process.execPath, ["--watch", "server/index.ts"], { stdio: "inherit" });
