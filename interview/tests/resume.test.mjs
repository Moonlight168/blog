...
test("HTML 简历显示为 .md，但选中值仍是真实路径", () => {
  ...
  assert.equal(html.name, "zhangsan/张三-Java后端.md");
  assert.ok(html.path.endsWith("张三-Java后端.html"));
});

test("isAllowedResume 只放行目录内的简历文件", () => {
  const root = makeResumeRoot();
  const inside = path.join(root, "zhangsan", "张三-Java后端.html");
...
test("HTML 转 markdown：章节成为标题、噪音被剥离、正文不丢", () => {
  const root = makeResumeRoot();
  const markdown = readResume(path.join(root, "zhangsan", "张三-Java后端.html"));
...
test("markdown / txt 原样读取，不做转换", () => {
  const root = makeResumeRoot();
  const text = readResume(path.join(root, "zhangsan", "个人优势.txt"));
