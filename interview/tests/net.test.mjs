...
import { friendlyNetworkError, httpStatusHint } from "../server/net.mjs";
...
test("状态码提示直接指向该改什么", () => {
  assert.match(httpStatusHint(401), /API Key/);
  assert.match(httpStatusHint(402), /余额/);
  assert.match(httpStatusHint(429), /频繁|额度/);
  assert.match(httpStatusHint(503), /服务端异常/);
