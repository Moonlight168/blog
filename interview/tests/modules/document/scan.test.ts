...
import { htmlToMarkdown, readResume, scanJobs, scanResumes } from "../../../server/modules/document/scan.ts";

/**
 * 简历目录的扫描与读取。
 *
 * 目录形状（与线上一致）：一级目录 = 一个人，里面是简历本体与一些**不该被当成简历**的邻居
 * （同名的 txt 副本、面试稿、个人优势、照片、规范文档）；JD 单独放在 jobs/ 下。
 */
...
<div class="proj-item"><div class="row-item"><div class="row-left"><strong>订单中台（智能流程设计系统） - 全栈开发</strong><span class="proj-tag">(毕业设计)</span></div><div class="row-right">2026.03-2026.06</div></div>
<div class="duty-label">项目职责：</div>
<ol><li><strong>Agent 工作流：</strong>基于 LangGraph 构建状态图</li></ol></div>
</body></html>`;

function makeResumeRoot(): string {
...
test("只列「姓名-岗位-届」命名的 HTML 简历本体，其余文件都不入列", () => {
  ...
  assert.deepEqual(names, ["zhangsan/张三-Java后端-27届.md"]);
...
test("HTML 简历显示为 .md，但选中值仍是真实路径", () => {
  ...
  assert.equal(html.name, "zhangsan/张三-Java后端-27届.md");
  assert.ok(html.path.endsWith("张三-Java后端-27届.html"));
});

test("HTML 转 markdown：章节成为标题、噪音被剥离、正文不丢", () => {
  const root = makeResumeRoot();
  const markdown = readResume(path.join(root, "zhangsan", "张三-Java后端-27届.html"));
  ...
  // 正文实体必须还在
  for (const kept of ["张三", "某大学", "计算机科学与技术", "订单中台", "LangGraph", "求职意向"]) {
...
<p>项目经历：订单中台</p></body></html>`;
  ...
  assert.ok(markdown.includes("订单中台"), "正文必须保留");
});

test("markdown / txt 原样读取，不做转换", () => {
  const root = makeResumeRoot();
  const text = readResume(path.join(root, "zhangsan", "张三-AI应用-27届.txt"));
