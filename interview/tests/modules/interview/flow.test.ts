...
import { advanceInterview, handleInterviewMessage } from "../../../server/modules/interview/flow.ts";
...
const session = (over: Partial<SessionWithFocus> = {}): SessionWithFocus =>
  makeSession({ id: "s1", status: "active", currentQuestion: { ...QUESTION }, ...over });

interface EngineOptions {
  classifyAction?: "answer" | "followup";
  evaluateResult?: { score: number; comment: string };
  archiveResult?: Record<string, unknown>;
  archiveError?: Error | null;
  summarizeResult?: string;
  attempts?: Array<{ title: string; score: number }>;
}
...
function makeEngine({
  ...
  const listAttempts = (id: string) => { calls.push("listAttempts:" + id); return attempts; };
  const engine = {
    handle: (target: SessionWithFocus, text: string) =>
      handleInterviewMessage({ session: target, text, piAgent, agent, archive, listAttempts }),
    advance: (target: SessionWithFocus, ending: boolean) =>
      advanceInterview({ session: target, ending, agent, listAttempts }),
    listAttempts,
  };
  return { engine, calls };
}
...
test("说「不知道」和普通回答走同一条路，只是分数低", async () => {
  ...
  assert.deepEqual(result.messages.map((m) => m.kind), ["evaluation", "answer"]);
  // Message.evaluation 在共享类型里是 unknown（DB 里存的是 JSON 串），这里按实际形状取
  const evaluation = result.messages[0].evaluation as Evaluation | undefined;
...
test("按钮：结束会带上本场逐题得分生成总评", async () => {
  ...
  const received = await engine.advance(session({ completedCount: 2 }), true);
...
test("书面测评：按钮推进整卷，答完最后一题自动结束", async () => {
  ...
  const { engine } = makeEngine();
  const base = makeSession({ id: "s1", status: "active", mode: "written", paperQuestions: paper, paperIndex: 0 });
