...
import { chapterFocuses, ensurePlan, focusPayload, planKey, resumeSlice, takeFocuses } from "../../../server/modules/interview/plan.ts";
...
function seedPlan(db: Db, key: string, labels: string[], cursor = 0) {
...
test("计划键：换简历、换 JD、换章节都是另一条进度线", () => {
  ...
  const chapter = makeSession({ mode: "interview", resumePath: "a.html", chapterPath: "Java/多线程.md" });
...
test("考点跨场推进，一轮走完自动进入第二轮", () => {
  ...
  const rounds: number[] = [];
  for (let i = 0; i < 3; i += 1) {
    const [focus] = takeFocuses(db, { key, plan }, 1);
    assert.ok(focus?.label, "每一轮都该取到一个考点");
    labels.push(focus.label);
    rounds.push(focus.round ?? 0);
  }
...
test("简历分段：按 anchor 命中该段，命中不到返回空串", () => {
  const resume = "# 某某\n\n## 专业技能\nJava、Spring\n\n## 项目经历\n1. 订单中台（流程设计）\n全栈开发\n2. 库存服务（指令助手）\nAgent 开发\n";
  const hit = resumeSlice(resume, { anchor: "库存服务", section: "项目经历" });
  assert.match(hit, /Agent 开发/);
  assert.doesNotMatch(hit, /订单中台/, "只截该考点那一段，别把兄弟项目也带上");
