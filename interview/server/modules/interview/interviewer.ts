import { apiUrl } from "../../infra/config/index.ts";
import { MAX_PROSE_LINES } from "../archive/format.ts";
import { friendlyNetworkError, httpStatusHint } from "../../infra/network.ts";
import type { ServiceConfig } from "../../infra/config/index.ts";
import type { Focus, Session, SessionWithFocus, TopicSeries } from "../../../shared/types.ts";

/**
 * 出题只需要「库里有哪些分类与章节」—— 按结构收窄，不要求整个题库索引。
 * 这样调用方（含测试）不必为了问一道题去建库、刷索引、配向量服务。
 */
 ...
 */
/** 限流与 5xx 之外的重试没意义，用这个标记告诉上层「别再试了」 */
type RetryableError = Error & { retryable?: boolean };

/** JSON 解析失败时把模型原文挂上去，调用方要能分清「没按 JSON 回」和「JSON 坏了」 */
type JsonError = Error & { raw?: string };
 ...
 */
function askedBlock(session: SessionWithFocus): string {
...
function focusBlock(next: Focus | undefined): string {
  if (!next?.label) return "";
  const round = (next.round ?? 1) > 1 ? `，第 ${next.round} 轮` : "";
  return `本轮要考察的考点（第 ${next.index}/${next.total} 个${round}）：${next.label}\n`
    + (next.section ? `出自简历板块：${next.section}\n` : "")
    + (next.intent ? `考察意图：${next.intent}\n` : "")
    + ((next.round ?? 1) > 1 ? `这个考点前面已经问过一轮，这次换一个更深的角度，不要再问同一个事实性问题。\n` : "")
    + `只围绕这一个考点出题，不要跳到别的板块。\n`;
}

/** 把本轮要覆盖的多个考点写成清单（书面测评一次出多题时用） */
function focusListBlock(list: Focus[] = []): string {
...
function resumeBlock(session: SessionWithFocus): string {
...
/** 要结构化结果的对话模型客户端：出题、判重、拆解简历、按规范重写标准答案、总结改动都走它，回的是 JSON */
export class InterviewAgent {
  readonly config: ServiceConfig;
  readonly questionIndex: TopicSource;

  constructor({ config, questionIndex }: { config: ServiceConfig; questionIndex: TopicSource }) {
    this.config = config;
   ...
   */
  async #chat(messages: any[], temperature = STABLE_TEMPERATURE, options: any = {}): Promise<any> {
    ...
    for (let attempt = 1; attempt <= 2; attempt += 1) {
      try {
        ...
        lastError = error;
        console.warn(JSON.stringify({ event: "ai_interaction", operation: options.operation ?? "chat", status: "error", attempt, retryable: (error as RetryableError).retryable !== false }));
        if ((error as RetryableError).retryable === false) break;
      }
    }
    throw lastError;
  }
  ...
  async #chatOnce(messages: any[], temperature = STABLE_TEMPERATURE, { operation = "chat", maxTokens = 1_600, attempt = 1 }: { operation?: string; maxTokens?: number; attempt?: number } = {}): Promise<any> {
    ...
    try {
      ...
      if ((error as RetryableError)?.name === "AbortError") {
        const timeout = new Error("对话模型响应超时，请重试") as RetryableError;
        timeout.retryable = true;
        throw timeout;
      }
      // 网络不可用：别把 undici 的 "fetch failed" 直接甩给用户
      throw new Error(friendlyNetworkError(error, { what: "对话模型服务", url }) ?? (error as Error).message);
    } finally { clearTimeout(timer); }
    if (!response.ok) {
      const error = new Error(`对话模型返回 HTTP ${response.status}${httpStatusHint(response.status)}：${(await response.text()).slice(0, 300)}`) as RetryableError;
      error.retryable = response.status === 429 || response.status >= 500;
      throw error;
    }
    let payload: ChatPayload;
    try { payload = (await response.json()) as ChatPayload; }
    ...
    try {
      ...
      (error as JsonError).raw = content;
      throw error;
    }
  }

  /** 单轮的便捷写法：一段系统提示 + 一句用户话 */
  async #json(system: string, user: string, temperature = STABLE_TEMPERATURE, options: any = {}): Promise<any> {
   ...
   * 面试官要听的是你怎么把事情讲下来。
   */
  async #generateHrQuestion({ session }: { session: SessionWithFocus }): Promise<unknown> {
  ...
  async generateQuestion({ session }: { session: SessionWithFocus }): Promise<any> {
   ...
   */
  async outlineResume({ session }: { session: SessionWithFocus }): Promise<unknown> {
    return this.#json(
  ...
  async generatePaper({ session, count }: { session: SessionWithFocus; count: number }): Promise<any[]> {
  ...
  async reformatAnswer({ question, errors = [], prose = false }: {
    question: { title?: string; standardAnswer?: string } & Record<string, unknown>;
    errors?: string[];
    prose?: boolean;
  }): Promise<string> {
    const shape = prose
  ...
  async judgeDuplicate({ title, candidates }: { title: string; candidates: unknown[] }): Promise<{ kind: string; questionId?: string }> {
  ...
  async summarize({ session, attempts = [] }: {
    session: SessionWithFocus; attempts?: Array<{ title: string; score: number; comment: string }>;
  }): Promise<string> {
