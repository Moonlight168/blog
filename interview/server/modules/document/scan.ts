...
import { isResumeFile } from "../../infra/paths.ts";

/**
 * 扫描面试用的简历与 JD，并把 HTML 简历转成 Markdown。
 *
 * 注意与「简历编辑稿」（resume.ts）的区别：这里处理的是**喂给模型的源文件**，
 * 那边是编辑器里可版本化的 HTML 稿。
 */
...
/**
 * 「已有简历」只列**简历本体**：命名符合 `姓名-目标岗位-毕业年限`（如 `张三-Java后端(AI应用)-27届`）
...
function isListedResume(file: string): boolean {
 ...
 */
export function scanResumes(root: string) {
...
function collect(directory: string, root: string, results: Array<{ name: string; file: string }>): void {
  for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
    if (entry.name.startsWith(".")) continue;
    const full = path.join(directory, entry.name);
    if (entry.isDirectory()) collect(full, root, results);
    else if (entry.isFile() && isListedResume(full)) {
      results.push({ name: path.relative(root, full).split(path.sep).join("/"), file: full });
    }
  }
}

/** 扫描 JD 目录（平铺，不按人分组）；显示名去掉文件后缀。 */
export function scanJobs(root: string) {
  ...
  return fs.readdirSync(dir, { withFileTypes: true })
    ...
    .map((entry) => ({ name: entry.name.replace(/\.[^.]+$/, ""), path: path.join(dir, entry.name) }))
    .sort((left, right) => left.name.localeCompare(right.name, "zh"));
}
 ...
 */
export function readResume(file: string): string {
...
export function htmlToMarkdown(html: string): string {
  ...
  service.addRule("entryRow", {
    ...
    replacement: (_content, node: any) => {
      ...
      const parts = left
        ? [...left.querySelectorAll("strong, span")].map((item: any) => item.textContent.trim()).filter(Boolean)
        : [];
