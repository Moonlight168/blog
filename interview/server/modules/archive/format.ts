...
export function widthUnits(text: string): number {
  ...
  for (const char of String(text)) {
    ...
    units += (char.codePointAt(0) ?? 0) < 0x7f ? 0.5 : 1;
  }
  return units;
}

/** 一级要点的「主句」＝ 第一个冒号之后到行尾（与《格式规范》口径一致） */
function mainClause(line: string): string {
  return line.split(/[：:]/u).slice(1).join(":").trim();
}

const isBalancedCode = (text: string): boolean => ((text.match(/`/g) ?? []).length % 2 === 0);
...
function safeCut(text: string, limit: number): number {
  ...
  for (let index = 0; index < text.length; index += 1) {
    ...
    if (char !== "`" && !/\s/u.test(char)) {
      const cost = (char.codePointAt(0) ?? 0) < 0x7f ? 0.5 : 1;
  ...
  const lastFitting = (cuts: number[]): number => [...cuts].reverse().find((cut) => isBalancedCode(text.slice(0, cut))) ?? 0;
 ...
 */
export function splitLongBullets(answer: string, limit: number = MAX_MAIN_UNITS): string {
  const out = [];
...
export function parseQuestions(markdown: string, sourcePath: string) {
 ...
 */
export function validateQuestionBlock(block: string, { prose = false, allowStar = false }: { prose?: boolean; allowStar?: boolean } = {}): string[] {
  const errors = [];
...
export function buildQuestionBlock({ title, answer, historyUrl, prose = false, allowStar = false }: {
  title: string; answer: string; historyUrl: string | null; prose?: boolean; allowStar?: boolean;
}): string {
