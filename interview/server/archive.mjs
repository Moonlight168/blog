...
export function createArchive({ questionIndex, agent, knowledgeRoot, privateHistoryRoot, db }) {
  return async ({ session, question, rawAnswer, evaluation }) => {
    ...
    const exact = candidates.find((candidate) => candidate.title === question.title);
    ...
    const historyOriginal = fs.existsSync(history.file) ? fs.readFileSync(history.file, "utf8") : "";
    const historyNext = mergeHistory(historyOriginal, { chapter, title: existing?.title ?? question.title, date, rawAnswer });
    let publicOriginal = null;
    try {
      if (!existing) {
        ...
        publicOriginal = appendAtomically(sourceFile, `\n${block}`);
      }
      ...
      if (publicOriginal !== null) fs.writeFileSync(sourceFile, publicOriginal, "utf8");
      throw error;
    }
    db.prepare("INSERT INTO attempts(session_id,question_title,raw_answer,evaluation,created_at) VALUES(?,?,?,?,?)")
      .run(session.id, question.title, rawAnswer, JSON.stringify(evaluation), new Date().toISOString());
