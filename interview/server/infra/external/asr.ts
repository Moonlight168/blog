import { apiUrl } from "../config/index.ts";
...
export function asrEnabled(asrConfig: ServiceConfig): boolean {
...
export async function transcribe({ config: asrConfig, buffer, mime, fetchImpl = fetch, timeoutMs = 60_000 }: {
  config: ServiceConfig; buffer: Buffer; mime: string;
  ...
  // Uint8Array 视图而不是裸 Buffer：TS 的 BlobPart 不认 Node 的 Buffer 类型，实际字节一样
  form.append("file", new Blob([new Uint8Array(buffer)], { type: contentType }), `speech.${extension}`);
  ...
  try {
    ...
    if ((error as Error)?.name === "AbortError") throw new Error("语音识别超时，请重试或改用浏览器内置识别");
    // 网络不通时给出「连不上谁」而不是 "fetch failed"
    throw new Error(friendlyNetworkError(error, { what: "语音识别服务", url }) ?? (error as Error).message);
  } finally {
    clearTimeout(timer);
  }
}
