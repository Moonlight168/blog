...
/** undici 把根因放在 error.cause —— 它不在 Error 的标准类型里，所以自己声明一份 */
interface NetworkCause { code?: string; message?: string }
interface NetworkError { message?: string; cause?: NetworkCause }
...
function causeText(error: unknown): string {
  const cause = asNetworkError(error).cause;
  if (!cause) return "";
 ...
 */
export function friendlyNetworkError(error: unknown, { what = "外部服务", url = "" }: {
  what?: string; url?: string;
} = {}): string | null {
  const detail = `${asNetworkError(error).message ?? ""} ${causeText(error)}`;
  if (!NETWORK_LIKE.test(detail)) return null;
  let host = url;
  try { host = new URL(url).host; } catch { /* 不是完整 URL 就原样用 */ }
  const reason = describeCause(error) || asNetworkError(error).message || "网络请求失败";
