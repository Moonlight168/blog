...
export class InterviewAgent {
  ...
  async generateQuestion({ session }) {
    ...
    const result = await this.#json(
      `你是严格的中文技术面试官。围绕指定章节并结合简历生成一道新的、真实面试口吻的问题，不编号、不加星标、一次只问一个核心任务。标准答案必须遵守《面试宝典文章格式规范》：1到6个“数字. **关键词**：简短主句”的一级要点，可用恰好3个空格缩进的“-”作为二级补充；不得使用三级标题；总计不超过15行。只返回 JSON：{"title":"以？结尾的问题","prompt":"向候选人展示的问题","standardAnswer":"标准答案"}。\n当前会话规则快照：\n${session.skillSnapshot}`,
  ...
  async evaluate({ question, rawAnswer }) {
    const result = await this.#json(
