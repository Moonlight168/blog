...
function fallbackSummary(attempts) {
...
export async function handleInterviewMessage({ session, text, piAgent, agent, archive, listAttempts, history = "", onEvent, signal }) {
  ...
  const decision = await piAgent.run({ session, text: input, history, onEvent, signal });
  ...
  onEvent?.({ type: "evaluation", evaluation });
  onEvent?.({ type: "stage", stage: "archiving" });
  const archived = await archive({ session, question: session.currentQuestion, rawAnswer: input, evaluation });
  if (signal?.aborted) {
  ...
  return {
    ...
    messages: [
      ...
      ...(archived?.notice ? [{ role: "assistant", kind: "notice", content: `${archived.notice}。这次回答已记录，可在「复习」里看到。` }] : []),
      { role: "assistant", kind: "answer", content: session.currentQuestion?.standardAnswer || "（这道题暂无标准答案）" },
    ],
  };
}

export async function advanceInterview({ session, ending, agent, listAttempts }) {
  const messages = [];
